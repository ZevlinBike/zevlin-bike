...
import { getRates as ssGetRates } from "@/lib/shipstation";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Package
    let pkg: { id?: string; name: string; length_cm: number; width_cm: number; height_cm: number; weight_g: number } | null = null;
    if (body.packageId) {
      const { data } = await supabase
        .from("shipping_packages")
        .select("id,name,length_cm,width_cm,height_cm,weight_g")
    ...
    // Weight: use same assumption as Shippo route (200g per item when unspecified)
    ...
    const itemsWeight = items.reduce((sum, li) => {
      ...
      if (weight && unit) {
        let g = 0;
        switch (unit) {
          case 'g': g = weight; break;
          case 'oz': g = weight * 28.3495; break;
          case 'lb': g = weight * 453.592; break;
          case 'kg': g = weight * 1000; break;
        }
    ...
    const ssRates = await ssGetRates({ to, from: ORIGIN, parcel });
    const rates = ssRates.map(r => ({
      rateObjectId: `${r.carrierCode}:${r.serviceCode}`,
      carrier: r.carrierCode,
      service: r.serviceName || r.serviceCode,
      amountCents: Math.round((r.shipmentCost || 0) * 100),
      currency: r.currency || 'USD',
      estimatedDays: r.deliveryDays ?? undefined,
    }));

    return NextResponse.json({ rates, totalWeightG });
  } catch (err) {
    const msg = err instanceof Error ? err.message : 'Failed to fetch ShipStation rates';
